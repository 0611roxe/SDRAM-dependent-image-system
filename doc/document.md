...
8.   fix图像显示存在白条/噪点的问题：
     ...
     3.   解决方案：让SDRAM初始化的结束信号作为VGA驱动的复位信号，只有当SDRAM完全准备好后才允许VGA去获取数据。注意，这里不是因为读写速度不平衡导致，原因在我们使用自动读写FIFO保证了数据是批量写入和读出的，也就是说不存在单个像素点的读写速度差异。当然，如果大家在测试过程中发现了读写速度问题导致的显示BUG，请联系我。
